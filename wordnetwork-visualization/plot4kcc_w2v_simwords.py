...
def visualization(G,imageFileName,nodecolor='#1f78b4'):
		...
		# nodes
		nx.draw_networkx_nodes(G,pos,node_size=1000,node_color=nodecolor)
		
		# edges
		nx.draw_networkx_edges(G,pos,edgelist=elarge,
				width=1.2,edge_color='blue')
		nx.draw_networkx_edges(G,pos,edgelist=esmall,
				width=1.2,alpha=0.5,edge_color='r',style='dashed')
...
def setEdges_simWords(model,word,n=10):
		simWords = model.wv.most_similar(word, topn=n)
		...
		for (w2,wgt) in simWords:
				simWords2 = model.wv.most_similar(w2, topn=5)
...
#		model_name = "D:/KCC_word2vec/model/FastText-KCC150.model"
		# model_name = "/word2vec-output-mecab.model"
		...
		G = setEdges_simWords(model,"편향",n=15)
		fileName = "word-network2.png"	# filename to save a image
		visualization(G,fileName,nodecolor='r')
